...
Is this borne out in practice? Maybe; in the functional world I notice a wider range of "micro-libs", a handful of classes bundled up as a project like `libfoo-bar`, with a limited scope ("extends `libfoo` to support `libbar`"). Certainly in OO-land there is at least the possibility that `libfoo`, `libbar` and `libfoo-bar` would be released together as a single, all-encompassing framework, like Spring or Rails. 