...
What's a `Bat`, in an OO program? If we declare it `extends Bird`, then it *is* a `Bird`. We can see from its class what methods it has; it implements `.fly()` and the rest of the `Bird` interface. How do we know what I can do with a `Taco`? In the functional  could be just about anywhere - but if all my important `Taco`-manipulation functions are on the `Taco` itself, as *methods*, then knowing what something *is* - its very essence - gives me an enormous amount of power and control.
