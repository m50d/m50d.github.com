...
C# has a syntactic shortcut for properties with trivial getters; I predict that Java will gain a similar feature in the not too distant future. But even if we adopted the C# feature, data classes in Java would still more verbose than Scala's case classes, and the latter can be used today.