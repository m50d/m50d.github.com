...
````
If we `print` a `Cat`, the library will use its own `CatPrinter`. If we try and print a `Leopard`, our code won't compile - but we can write our own `LeopardPrinter` to handle that case, and we can do this without talking to the printing team or the safari team.
