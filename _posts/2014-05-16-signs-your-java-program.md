...
I feel obliged to say a few words on complexity, since most Scala discussions seem to end up there. Scala is a consistent language (more so than Java - e.g. primitives, arrays and operators are much less of a special case in Scala) with a relatively short specification, but it contains many very general features that can interact in complex ways; sometimes 
