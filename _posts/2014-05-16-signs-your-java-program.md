...
Even the best annotation-driven extensions make it harder to reason about the code by reading it, and present a barrier to developers joining the project. (Of course, a skilled developer should be able to pick up a fresh extension fairly quickly - but the same could be said of picking up a new language). Look at something like Jersey - a well-designed, well-regarded framework for REST APIs in Java, and a great improvement over what has gone before. 
