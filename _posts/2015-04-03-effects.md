 ...
 * How an effect appears locally in the source code. Some effects are *magic* (that is, completely invisible), in which case most of the other criteria become irrelevant - the language simply manages that effect for you. Some are *concise* and some are *verbose*.
...
#The examples
##Memory management
Java, Python, Scala: magic, implicit, safe. Most languages these days have automatic garbage collection. Most - but by no means all - of the time, performance is acceptable. Any function might allocate or free memory.
