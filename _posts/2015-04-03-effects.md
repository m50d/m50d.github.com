...
Java <=6, Python <= 2.4: verbose, implicit, unsafe. The programmer must explicitly free resources, and it usually requires a  `try`/`finally` block to do correctly.
Java 7+, Python 2.5+: concise, implicit, safe, using try-with-resources or `with` statements.  
