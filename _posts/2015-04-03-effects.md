...
Java using Spring AOP or AspectJ, Python using decorators: concise, single-function, unsafe. We saw these approaches in the database section, but it's possible to use them for custom effects - it's very easy in Python since a decorator is just a function, and a little more complex but very much doable in Java.
Scala using a custom monad: concise, generic, semi-safe. This is where the generics-based approach really shines: in Scala a custom effect is just another effect, we can reuse existing functions (e.g. scalaz's `sequence` can combine a list of `Future`s, or a list of Doobie database accesses, or a list of )
#Thoughts
