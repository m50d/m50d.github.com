...
Java with Hibernate/JPA, Python with Django: magic, implicit/???, safe. These approaches offer two possibilities for transaction boundaries: either method scoped via an `@Transactional`, or session-in-view (i.e. one transaction per web request).
