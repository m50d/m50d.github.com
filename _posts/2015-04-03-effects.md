...
C++: semi-concise, semi-generic, semi-safe. C++ can often feel like several languages at once, 
Using stack-allocated objects, smart pointers and so forth. It’s up to the programmer to use these features, but if used correctly they provide correctness guarantees. RAII means memory and non-memory resources can be managed in the same way (so e.g. templates can be polymorphic in what kind of resource they’re using). Dedicated language constructs - pure stack-allocation/RAII (i.e. no shared objects) would be in the elegant category, but probably too hard to use in pratical programs.
