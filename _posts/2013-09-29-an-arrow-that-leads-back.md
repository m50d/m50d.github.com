...
But there's a part of me that thinks: when I started using it, scala was a bondage-and-discipline language. The straightjacket of typing and immutability made it harder to write certain kinds of programs - but it also let me stretch further than before, into higher and higher levels of genericity, without risk of hurting myself. It makes me wonder how much better this approach really is than, say, a python implementation using some stack-slicing greenlet-style system, where only the function names would indicate that they were or weren't making async calls.
...
[3] To get the async http behaviour we'd need to use some kind of stack-slicing greenlet-style async framework - I understand there are at least two popular ones