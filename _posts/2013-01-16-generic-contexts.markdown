...
This is generic enough to be used for many other operations besides our "sequence" (e.g. it's almost trivial to implement map for a generic C\[_\]:ComposeableContext), but it's fairly easy to use: we'll write a "contextless" addToTL function, and then use a ComposeableContext and the .curried builtin to turn it into one whose parameters are inside our context C:
